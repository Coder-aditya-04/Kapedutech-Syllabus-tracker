...
export default async function HeadLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen relative">
      {/* ── Floating background blobs (hidden on mobile for performance) ── */}
      ...
      <header className="sticky top-0 z-50 nav-glow" style={{ background: 'linear-gradient(90deg, #08090A 0%, #0f0a1e 50%, #08090A 100%)' }}>
        ...
        <div className="flex items-center px-4 md:px-8 h-16 md:h-[88px] gap-3 md:gap-8 max-w-screen-2xl mx-auto">
          ...
          <div className="shrink-0 flex items-center gap-2 md:gap-3">
            ...
            <MobileNav
              ...
              initials={initials}
              name={profile.name}
              role={profile.role === 'director' ? 'Director' : 'Head'}
            />
          </div>
        </div>
      </header>

...
interface QFile { id: string; file_url: string; file_name: string; file_type: string; is_fair_copy: boolean }
interface Upload {
...
interface Props {
  ...
  chapters: Array<{ subject: string; chapter_name: string }>
}
...
export default function QuestionUploadForm({ assignments, recentUploads, chapters }: Props) {
  ...
  const chapterSuggestions = chapters.filter(c => c.subject === form.subject).map(c => c.chapter_name)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      let uploadedFiles: QFile[] = []
      if (files.length > 0) {
        setUploadingFiles(true)
        uploadedFiles = await uploadFilesViaServer(upload.id)
    ...
    <div className="min-h-screen pb-12" style={{ background: '#ede9f9' }}>
      ...
      <div className="relative max-w-screen-xl mx-auto px-4 md:px-8 pt-8">
        <div className="mb-8">
          ...
          <p className="text-gray-500 text-base mt-1">Upload your daily questions with chapter and sub-topic</p>
        </div>
          ...
          <div className="lg:col-span-2">
            <form onSubmit={handleSubmit}
              ...
              <div>
                ...
                <select value={form.batch_id} onChange={e => { set('batch_id', e.target.value); set('subject', assignments.find(a => a.batch_id === e.target.value)?.subject ?? '') }}
              ...
              <div>
                ...
                <select value={form.subject} onChange={e => { set('subject', e.target.value); set('chapter_name', '') }}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-800 bg-white focus:ring-2 focus:ring-violet-400 focus:border-transparent outline-none">
                  {subjectsForBatch.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>

              {/* Chapter — datalist for autocomplete from previous uploads */}
              <div>
                ...
                <datalist id="chapter-suggestions">
                  {chapterSuggestions.map((c, i) => <option key={i} value={c} />)}
                </datalist>
              </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        {roughFiles.map(f => (
                          <a key={f.id} href={f.file_url} target="_blank" rel="noopener noreferrer"
                            ...
                            {f.file_type === 'pdf' ? '📄' : f.file_type === 'docx' ? '📝' : '🖼️'} {f.file_name}
                          </a>
                        ))}
                      </div>
                    )}
